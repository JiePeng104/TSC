...
def replace_record_strings(d, record_name):
    for k, v in d.items():
        if isinstance(v, dict):
            replace_record_strings(v, record_name)
        elif isinstance(v, str) and 'record/' in v:
            suffix = v.split('record/')[1]
            suffix_record_name = suffix.split('/')[0]
            suffix = suffix.replace(suffix_record_name, record_name)
            d[k] = f'/mnt/BackdoorBench-main2/record/{suffix}'
    return d
...
def load_attack_result(
    ...
    record_name = save_path.split('record/')[1].split('/')[0]
    ...
    if all(key in load_file for key in ['model_name',
        ...
        if not os.path.exists(clean_setting.dataset_path):
            clean_setting.dataset_path = f'/mnt/BackdoorBench-main/data/{clean_setting.dataset}'
        ...
        if load_file['bd_train'] is not None:
            ...
            if not os.path.exists(load_file['bd_train']['save_folder_path']):
                replace_record_strings(load_file['bd_train'], record_name)
        ...
        if not os.path.exists(load_file['bd_test']['save_folder_path']):
            replace_record_strings(load_file['bd_test'], record_name)
