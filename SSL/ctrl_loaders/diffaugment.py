...
class PoisonAgent():
    ...
    def choose_poisons_randomly(self):
        ...
        if self.args.threat_model == 'our':
            ...
            x_train_tensor[poison_index], y_train_tensor[poison_index] = self.fre_poison_agent.Poison_Frequency_Diff(
                x_train_tensor[poison_index], y_train_tensor[poison_index], self.magnitude)
            x_test_pos_tensor, y_test_pos_tensor = self.fre_poison_agent.Poison_Frequency_Diff(
                x_test_tensor.clone().detach(), y_test_tensor.clone().detach(), self.magnitude)
        else:
        ...
        filtered_mask = y_test_pos_tensor != self.args.target_class
        import logging
        logging.info(f'before filtering.....{len(y_test_pos_tensor)}')
        x_test_pos_tensor = x_test_pos_tensor[filtered_mask]
        y_test_pos_tensor = y_test_pos_tensor[filtered_mask]
        logging.info(f'after filtering.....{len(y_test_pos_tensor)}')
        ...
        test_index_pos = test_index[filtered_mask]  # Filter the index tensor too
        memory_index = torch.tensor(list(range(len(x_memory_tensor))), dtype=torch.long)

        train_sampler = None

        train_loader = DataLoader(TensorDataset(x_train_tensor, y_train_tensor, train_index),
                                  batch_size=self.args.batch_size, sampler=train_sampler,
                                  shuffle=(train_sampler is None), drop_last=True)
        test_loader = DataLoader(TensorDataset(x_test_tensor, y_test_tensor, test_index),
                                 batch_size=self.args.eval_batch_size, shuffle=False, drop_last=True)
        test_pos_loader = DataLoader(TensorDataset(x_test_pos_tensor, y_test_pos_tensor, test_index_pos),
                                     batch_size=self.args.eval_batch_size, shuffle=False)
        memory_loader = DataLoader(TensorDataset(x_memory_tensor, y_memory_tensor, memory_index),
                                   batch_size=self.args.eval_batch_size, shuffle=False)
