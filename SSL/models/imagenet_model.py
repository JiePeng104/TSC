...
class ImageNetResNet_project(nn.Module):
    def __init__(self,
                 #  embed_dim: int,
                 #  # vision
                 #  image_resolution: int,
        ...
        super(ImageNetResNet_project, self).__init__()
        self.visual = ResNet(Bottleneck, [3, 4, 6, 3], width_mult=1)

        self.projection_model = nn.Sequential(nn.Linear(2048, 512, bias=False), nn.BatchNorm1d(512),
                                              nn.ReLU(inplace=True), nn.Linear(512, feature_dim, bias=True))

    def forward(self, x):
        feature = self.visual(x)
        out = self.projection_model(feature)
