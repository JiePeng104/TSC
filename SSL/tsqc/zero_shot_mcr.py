...
def load_bn_and_attention_params(model_a, model_b):
    ...
    logging.info("Finished loading BatchNorm and AttentionPool2d parameters from model B into model A.")
...
class mc_repair_Class():
    ...
    def zero_shot(self, args, model_curve, architecture, curve_name,
        ...
        # logging.info('curve_parameters')
        # for name, param in model_curve.net.named_parameters():
        #     if param.requires_grad:
        #         logging.info(
        #             f'Parameter: {name}, Shape: {param.shape}, Requires Grad: {param.requires_grad}, Value: {param.detach().cpu().numpy()}')

        # for k in range(args.num_bends):
        #     spmodel = architecture.base(num_classes=args.num_classes, **architecture.kwargs_base)
        #     logging.info(f'exported model {k}')
        #     model_curve.export_base_parameters(spmodel, k)
        #     for name, param in spmodel.named_parameters():
        #         if param.requires_grad:
        #             logging.info(
        #                 f'Parameter: {name}, Shape: {param.shape}, Requires Grad: {param.requires_grad}, Value: {param.detach().cpu().numpy()}')
        ...
        for k, t_value in enumerate(test_array):
            ...
            #                                           curve_model=model_curve, t=t, train_loader=encoder_train_loader,
            #                                           device=self.device,
            ...
            model = curve_opt.import_PointOnCurve_device(args=args, spmodel=model, architecture=architecture,
                                                         ...
                                                         device=self.device,
                                                         bn_updating=True if args.bn_updating > 0 else False)
            # model = architecture.base(num_classes=args.num_classes, **architecture.kwargs_base)
            # if torch.cuda.is_available():
            #     model = model.cuda()
            # curve_opt.update_bn(encoder_train_loader, model)
            #
            # logging.info(f'exported model {k}')
            # model_curve.export_base_parameters(model, k)
    ...
    def curvePoint_zero_shot(self, args, encoder_train_loader, train_loader, test_loader_clean,
        ...
        curve_acc_result, curve_asr_result = self.zero_shot(args, model, architecture,
        ...
        import pickle
        filename = f"zero_shot-RandomSeed_{args.random_seed}_CurvePoint_{args.curve_t}_Ratio_{args.ratio}_FixStart_{args.fix_start}_FixEnd_{args.fix_end}_Epoch_{args.epochs}_FtLR_{args.ft_lr}.pkl"
        if args.coco > 0:
