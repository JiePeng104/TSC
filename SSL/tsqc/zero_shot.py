...
def load_bn_and_attention_params(model_a, model_b):
    from models.clip_model import AttentionPool2d
    # 1. 加载模型B的BatchNorm和AttentionPool2d参数到模型A
    model_b_bn_params = {}
    model_b_attnpool_params = {}

    # 提取模型B中的BatchNorm和AttentionPool2d参数
    for name, module in model_b.named_modules():
        if isinstance(module, nn.BatchNorm2d):
            model_b_bn_params[name] = module.state_dict()
        elif isinstance(module, AttentionPool2d):
            model_b_attnpool_params[name] = module.state_dict()

    # 2. 将模型B的BatchNorm参数加载到模型A中
    for name, module in model_a.named_modules():
        if isinstance(module, nn.BatchNorm2d):
            if name in model_b_bn_params:
                # 加载参数
                module.load_state_dict(model_b_bn_params[name])
            else:
                logging.info(f"Warning: {name} not found in model B")
    logging.info("Finished loading BatchNorm parameters from model B into model A.")

    # # 3. 将模型B的AttentionPool2d参数加载到模型A中
    # for name, module in model_a.named_modules():
    #     if isinstance(module, AttentionPool2d):
    #         if name in model_b_attnpool_params:
    #             # 加载参数
    #             module.load_state_dict(model_b_attnpool_params[name])
    #         else:
    #             logging.info(f"Warning: {name} not found in model B")

    # logging.info("Finished loading AttentionPool2d parameters from model B into model A.")
...
def logging_unified_results(args):
    dir_path = os.path.join(os.getcwd(), 'record/' + args.result_file + '/defense/mc_repair/results')
    if not os.path.exists(dir_path):
        os.makedirs(dir_path)

    import pickle
    filename = f"zero_shot-RandomSeed_{args.random_seed}_CurvePoint_{args.curve_t}_Ratio_{args.ratio}_FixStart_{args.fix_start}_FixEnd_{args.fix_end}_Epoch_{args.epochs}_TSPCEpoch_{args.TSPC_epoch}.pkl"
    if args.coco > 0:
        filename = 'coco-'+filename
    file_path = os.path.join(dir_path, filename)

    with open(file_path, 'wb') as file:
        pickle.dump((first_stage_curve_acc_result, second_stage_curve_acc_result,
...
class mc_repair_Class():
    ...
    def set_result(self, result_file):
        ...
        if self.args.log is None:
            self.args.log = save_path + 'log_zero_shot/'
    ...
    def mitigation(self):
        ...
        self.curvePoint_zero_shot(args, encoder_train_loader, train_loader, test_loader_clean,
    ...
    def zero_shot(self, args, model_curve, architecture, curve_name,
        ...
        if args.dataset == 'gtsrb':
            ...
            text_inputs = torch.cat(
                [clip.tokenize(f"A photo of a {c}, a type of food.") for c in test_loader_clean.dataset.classes]).to(self.device)
        else:
            raise NotImplementedError
        with torch.no_grad():
        ...
        for k, t_value in enumerate(test_array):
            ...
            #     model = self.load_BN_fromOriModel(args, model, ori_model)
                # model = self.mc_repair(args, ori_model, model,
                #                        encoder_train_loader, None,
                #                        maximize=True,
                #                        one_layer_per=False)
                # model = self.load_BN_fromOriModel(args, model, ori_model)
                # load_bn_and_attention_params(model, ori_model)
            ...
            print(f"Attack Success Rate: {bd_acc}")
    ...
    def curvePoint_zero_shot(self, args, encoder_train_loader, train_loader, test_loader_clean,
                             test_loader_backdoor, target_loader,
                             clean_clip=None):
        ...
        encoder_train_loader = torch.utils.data.DataLoader(encoder_train_loader.dataset.new_subset(0.05),
                                                           batch_size=min(args.batch_size, 128),
                                                           num_workers=args.num_workers, shuffle=True)

        for i in range(args.TSPC_epoch):
            # curve_name1 = f'{i}_round_First-Stage-t={t}'
            # curve_name2 = f'{i}_round_Second-Stage-t={t}'
            curve_name1 = f'{i}_round_First-Stage-t={t}' if (args.freezing == 0 or not (
                        args.encoder_usage_info == 'CLIP')) else f'{i}_round_First-Stage-freezing-t={t}'

            curve_name2 = f'{i}_round_Second-Stage-t={t}' if (args.freezing == 0 or not (
                        args.encoder_usage_info == 'CLIP')) else f'{i}_round_Second-Stage-freezing-t={t}'
            if args.coco > 0:
                curve_name1 = curve_name1 + '-coco'
                curve_name2 = curve_name2 + '-coco'

            cl_result1, adv_result1 = self.zero_shot(args, model, architecture,
                                                     f'curve_result_{curve_name1}-ratio{args.ratio}.pt',
                                                     encoder_train_loader, train_loader, test_loader_clean,
                                                     test_loader_backdoor, target_loader,
                                                     original_model,
                                                     clean_clip=clean_clip)

            cl_result2, adv_result2 = self.zero_shot(args, model, architecture,
                                                     f'curve_result_{curve_name2}-ratio{args.ratio}.pt',
                                                     encoder_train_loader, train_loader, test_loader_clean,
                                                     test_loader_backdoor, target_loader,
                                                     original_model,
                                                     clean_clip=clean_clip)
