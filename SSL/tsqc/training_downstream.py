...
def logging_unified_results(args):
    ...
    if 0 < args.middle_point < 1:
        filename = f'Middle_point_{args.middle_point}-' + filename
    ...
    def mitigation(self):
        ...
        if args.extra_classes > 0:  # loading extra target
            if 'truck' in args.reference_file:
                ...
                target_dataset.filter_by_target(target_dataset.classes.index('truck'))
                target_dataset.reset_target(args.reference_label)
                print(f'Reset target dataset STL10, target labe{target_dataset.labels[0]}')
        ...
        if args.encoder_usage_info == 'imagenet' and args.random_crop > 0:
            ...
            random_crop_transformation, _ = get_processing('imagenet', augment=False, size=224)
