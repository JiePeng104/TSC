...
def training_fc(args, model, clean_train_loader, device):
    if args.encoder_usage_info in ['stl10', 'cifar10', 'imagenet']:
        train_optimizer = torch.optim.Adam(model.parameters(), lr=0.00001, weight_decay=1e-6)
    else:
    ...
    ft_epoch = args.training_fc_epochs
...
def logging_unified_results_ssl(args):
    dir_path = os.path.join(os.getcwd(), 'record/' + args.result_file + '/defense/mc_repair/middle_results')
    if not os.path.exists(dir_path):
        os.makedirs(dir_path)

    import pickle
    filename = f"Middle_RandomSeed_{args.random_seed}_CurvePoint_{args.curve_t}_Ratio_{args.ratio}_FixStart_{args.fix_start}_FixEnd_{args.fix_end}_Epoch_{args.epochs}_TSPCEpoch_{args.TSPC_epoch}.pkl"
...
class mc_repair_Class():
    ...
    def add_arguments(parser):
        ...
        parser.add_argument('--dataset', type=str, help='cifar10, gtsrb, svhn, food101, voc2007')
        ...
        parser.add_argument('--wd', default=5e-5, type=float, help='weight decay of sgd')
    ...
    def mitigation(self):
        ...
        if args.downstream_test > 0 or args.encoder_usage_info == 'CLIP':
            ...
            if args.extra_classes > 0:  # loading extra target
                if 'truck' in args.reference_file:
                    ...
                    down_target_dataset.filter_by_target(down_target_dataset.classes.index('truck'))
                    down_target_dataset.reset_target(args.reference_label)
                    print(f'Reset target dataset STL10, target labe{down_target_dataset.labels[0]}')
        ...
        self.repair_train(args, model, train_clean_loader, test_clean_loader, down_test_loader_clean,
                          down_test_loader_backdoor, down_target_loader, down_train_loader, clean_clip=clean_clip)
    ...
    def mode_connectivity_Point(self, args, model_a, model_b, data_train, clean_test,
        ...
        if args.downstream_test > 0:
            ...
            for m, name in zip([model_a, point_model[0], model_b], [0, point_t, 1]):
                clean_acc, clean_loss, bd_acc, bd_loss = self.training_Downstream_single(args, m, str(name),
                                                                                         down_train_loader,
                                                                                         down_test_loader_clean,
                                                                                         down_test_loader_backdoor,
                                                                                         down_target_loader)
            ...
            if curve_name is not None and 'First' in curve_name:
                first_stage_curve_acc_result.append(clean_dict)
                first_stage_acc_result.append(clean_dict['acc'][1])
                first_stage_curve_asr_result.append(adv_dict)
                first_stage_asr_result.append(adv_dict['acc'][1])
            else:
                second_stage_curve_acc_result.append(clean_dict)
                second_stage_acc_result.append(clean_dict['acc'][1])
                second_stage_curve_asr_result.append(adv_dict)
                second_stage_asr_result.append(adv_dict['acc'][1])
    ...
    def repair_another(self, args, model, clean_train_loader, clean_test, down_test_loader_clean,
                       down_test_loader_backdoor, down_target_loader, down_train_loader, clean_clip=None):
        t = args.curve_t
        ...
        logging_unified_results_ssl(args)
