...
def find_permutation_ResNet18_woFC(model0, model1, val_loader, maximize=False):
    # First Conv
    class Subnet(nn.Module):
        def __init__(self, model):
            super().__init__()
    ...
    perm_map = get_layer_perm(Subnet(model0.f), Subnet(model1.f), val_loader, maximize=maximize)
    permute_output(perm_map, model1.f.conv1, model1.f.bn1)
    # make permutation for RESIDUAL Block
    permute_output(perm_map, model1.f.layer1[0].conv2, model1.f.layer1[0].bn2)
    permute_output(perm_map, model1.f.layer1[1].conv2, model1.f.layer1[1].bn2)
    permute_input(perm_map, [model1.f.layer1[0].conv1, model1.f.layer1[1].conv1,
                             model1.f.layer2[0].conv1, model1.f.layer2[0].downsample[0]])
    ...
    perm_map = get_layer_perm(Subnet(model0.f), Subnet(model1.f), val_loader, maximize=maximize)
    permute_output(perm_map, model1.f.layer1[0].conv1, model1.f.layer1[0].bn1)
    permute_input(perm_map, model1.f.layer1[0].conv2)
    ...
    perm_map = get_layer_perm(Subnet(model0.f), Subnet(model1.f), val_loader, maximize=maximize)
    permute_output(perm_map, model1.f.layer1[1].conv1, model1.f.layer1[1].bn1)
    permute_input(perm_map, model1.f.layer1[1].conv2)
    ...
    perm_map = get_layer_perm(Subnet(model0.f), Subnet(model1.f), val_loader, maximize=maximize)
    permute_output(perm_map, model1.f.layer2[0].conv1, model1.f.layer2[0].bn1)
    permute_input(perm_map, model1.f.layer2[0].conv2)
    ...
    perm_map = get_layer_perm(Subnet(model0.f), Subnet(model1.f), val_loader, maximize=maximize)
    permute_output(perm_map, model1.f.layer2[0].conv2, model1.f.layer2[0].bn2)
    permute_output(perm_map, model1.f.layer2[1].conv2, model1.f.layer2[1].bn2)
    permute_output(perm_map, model1.f.layer2[0].downsample[0], model1.f.layer2[0].downsample[1])
    permute_input(perm_map, [model1.f.layer2[1].conv1,
                             model1.f.layer3[0].conv1, model1.f.layer3[0].downsample[0]])
    ...
    perm_map = get_layer_perm(Subnet(model0.f), Subnet(model1.f), val_loader, maximize=maximize)
    permute_output(perm_map, model1.f.layer2[1].conv1, model1.f.layer2[1].bn1)
    permute_input(perm_map, model1.f.layer2[1].conv2)
    ...
    perm_map = get_layer_perm(Subnet(model0.f), Subnet(model1.f), val_loader, maximize=maximize)
    permute_output(perm_map, model1.f.layer3[0].conv1, model1.f.layer3[0].bn1)
    permute_input(perm_map, model1.f.layer3[0].conv2)
    ...
    perm_map = get_layer_perm(Subnet(model0.f), Subnet(model1.f), val_loader, maximize=maximize)
    permute_output(perm_map, model1.f.layer3[0].conv2, model1.f.layer3[0].bn2)
    permute_output(perm_map, model1.f.layer3[1].conv2, model1.f.layer3[1].bn2)
    permute_output(perm_map, model1.f.layer3[0].downsample[0], model1.f.layer3[0].downsample[1])
    permute_input(perm_map, [model1.f.layer3[1].conv1,
                             model1.f.layer4[0].conv1, model1.f.layer4[0].downsample[0]])
    ...
    perm_map = get_layer_perm(Subnet(model0.f), Subnet(model1.f), val_loader, maximize=maximize)
    permute_output(perm_map, model1.f.layer3[1].conv1, model1.f.layer3[1].bn1)
    permute_input(perm_map, model1.f.layer3[1].conv2)
    ...
    perm_map = get_layer_perm(Subnet(model0.f), Subnet(model1.f), val_loader, maximize=maximize)
    permute_output(perm_map, model1.f.layer4[0].conv1, model1.f.layer4[0].bn1)
    permute_input(perm_map, model1.f.layer4[0].conv2)
    ...
    #
    # perm_map = get_layer_perm(Subnet(model0.f), Subnet(model1.f), val_loader, maximize=maximize)
    # permute_output(perm_map, model1.f.layer4[0].conv2, model1.f.layer4[0].bn2)
    # permute_output(perm_map, model1.f.layer4[1].conv2, model1.f.layer4[1].bn2)
    # permute_output(perm_map, model1.f.layer4[0].downsample[0], model1.f.layer4[0].downsample[1])
    # permute_input(perm_map, model1.f.layer4[1].conv1)
    # # Make permutation for Linear Layer
    # model1.f.linear.weight.data = model1.f.linear.weight[:, perm_map]
    #
    class Subnet(nn.Module):
        def __init__(self, model):
            super().__init__()
    ...
    perm_map = get_layer_perm(Subnet(model0.f), Subnet(model1.f), val_loader, maximize=maximize)
    permute_output(perm_map, model1.f.layer4[1].conv1, model1.f.layer4[1].bn1)
    permute_input(perm_map, model1.f.layer4[1].conv2)
...
def find_permutation_ResNet50_woFC(model0, model1, val_loader, maximize=False):
    def get_model_resnet50_block(model):
        return nn.Sequential(nn.Sequential(model.f.conv1, model.f.bn1, model.f.relu),
                             *model.f.layer1, *model.f.layer2, *model.f.layer3, *model.f.layer4)
    ...
    perm_map = None
    kk_list = sum([[0], 3 * [3], 4 * [7], 6 * [13], 3 * [16]], [])
    for k in range(len(model0_block) - 1):
        kk = kk_list[k]
...
def find_permutation_ResNet50_imagenet(model0, model1, val_loader, maximize=False):
    # the resnet50 for imagenet is warped by ImageNetResNet
    def get_model_resnet50_block(model):
        return nn.Sequential(
            nn.Sequential(model.visual.conv1, model.visual.bn1, model.visual.relu, model.visual.maxpool),
            *model.visual.layer1, *model.visual.layer2, *model.visual.layer3, *model.visual.layer4)
    ...
    #             permute_output(perm_map, shortcut[0], shortcut[1])
    #     else:
    #         permute_output(perm_map, model1.visual.conv1, model1.visual.bn1)
    #
    #     permute_input(perm_map, model1_block[k + 1].conv1)
    #     shortcut = model1_block[k + 1].downsample
    #     if shortcut:
...
def find_permutation_ResNet50_imagenet_percent(model0, model1, val_loader, percent=0.5, maximize=False):
    # the resnet50 for imagenet is warped by ImageNetResNet
    def get_model_resnet50_block(model):
        return nn.Sequential(
            nn.Sequential(model.visual.conv1, model.visual.bn1, model.visual.relu, model.visual.maxpool),
    ...
    r1 = random.sample(range(1, len(model0_block)), int(len(model0_block)*percent))
    for k in range(1, len(model0_block)):
        if k in r1:
            continue
        block0 = model0_block[k]
    ...
    length2 = len(model0_block) - 1
    r2 = random.sample(range(1, length2), int(length2 * percent))
...
def find_permutation_ModifiedResNet50_CLIP(model0, model1, val_loader, maximize=False):
    def get_model_resnet50_block(model):
        return nn.Sequential(nn.Sequential(model.visual.conv1, model.visual.bn1, model.visual.relu,
                                           model.visual.conv2, model.visual.bn2, model.visual.relu,
                                           model.visual.conv3, model.visual.bn3, model.visual.relu,
                                           model.visual.avgpool),
                             *model.visual.layer1, *model.visual.layer2, *model.visual.layer3, *model.visual.layer4)
    ...
    # stem layer
    subnet0 = nn.Sequential(model0.visual.conv1, model0.visual.bn1, model0.visual.relu)
    subnet1 = nn.Sequential(model1.visual.conv1, model1.visual.bn1, model1.visual.relu)
    perm_map = get_layer_perm(subnet0, subnet1, val_loader, maximize=maximize)
    permute_output(perm_map, model1.visual.conv1, model1.visual.bn1)
    permute_input(perm_map, model1.visual.conv2)

    subnet0 = nn.Sequential(model0.visual.conv1, model0.visual.bn1, model0.visual.relu, model0.visual.conv2,
                            model0.visual.bn2, model0.visual.relu)
    subnet1 = nn.Sequential(model1.visual.conv1, model1.visual.bn1, model1.visual.relu, model1.visual.conv2,
                            model1.visual.bn2, model1.visual.relu)
    perm_map = get_layer_perm(subnet0, subnet1, val_loader, maximize=maximize)
    permute_output(perm_map, model1.visual.conv2, model1.visual.bn2)
    permute_input(perm_map, model1.visual.conv3)
    ...
    for k in range(1, len(model0_block)):
        ...
        # Add an avgpool after bn2
        # subnet0 = nn.Sequential(model0_block[:k],
        #                         block0.conv1, block0.bn1, block0.relu,
        #                         block0.conv2, block0.bn2, block0.relu, block0.avgpool)
        # subnet1 = nn.Sequential(model1_block[:k],
        #                         block1.conv1, block1.bn1, block1.relu,
        #                         block1.conv2, block1.bn2, block1.relu, block1.avgpool)
        subnet0 = nn.Sequential(model0_block[:k],
                                block0.conv1, block0.bn1, block0.relu,
                                block0.conv2, block0.bn2, block0.relu)
        subnet1 = nn.Sequential(model1_block[:k],
    ...
    #         shortcut = model1_block[k].downsample
    #         if shortcut:
    #             permute_output(perm_map, shortcut[1], shortcut[2])
    #     else:
    #         # the last layer of stem of CLIP-resnet is (conv3, bn3, avgpool)
    #         permute_output(perm_map, model1.visual.conv3, model1.visual.bn3)
    #
    #     permute_input(perm_map, model1_block[k + 1].conv1)
    #     shortcut = model1_block[k + 1].downsample
    #     if shortcut:
    #         permute_input(perm_map, shortcut[1])

    return model1


def find_permutation_ModifiedResNet50_CLIP_percent(model0, model1, val_loader, percent=0.5, maximize=False):
    def get_model_resnet50_block(model):
        return nn.Sequential(nn.Sequential(model.visual.conv1, model.visual.bn1, model.visual.relu,
                                           model.visual.conv2, model.visual.bn2, model.visual.relu,
                                           model.visual.conv3, model.visual.bn3, model.visual.relu,
    ...
    for i in range(n):
        ...
        if isinstance(layer, nn.Conv2d):
            ...
            if isinstance(feats1[i + 1], nn.BatchNorm2d):
                ...
                perm_map = get_layer_perm(subnet_features(model0, i + 2 + 1), subnet_features(model1, i + 2 + 1),
                                          val_loader, method=method,
                                          maximize=maximize)
                permute_output(perm_map, feats1[i], feats1[i + 1])
            else:
                assert isinstance(feats1[i + 1], nn.ReLU)
                perm_map = get_layer_perm(subnet_features(model0, i + 1 + 1), subnet_features(model1, i + 1 + 1),
                                          val_loader, method=method,
                                          maximize=maximize)
