...
class PositionalEmbedding(CurveModule):

    def __init__(self, parameter_tensor, fix_points):
        super(PositionalEmbedding, self).__init__(fix_points, ['parameter'])

        self.l2 = 0.0
        for i, fixed in enumerate(self.fix_points):
            self.register_parameter(
                'parameter_%d' % i,
                Parameter(parameter_tensor, requires_grad=not fixed)
            )

    def forward(self, input, coeffs_t):
        parameter_t = self.compute_weights_t(coeffs_t)[0]
        return input + parameter_t[:, None, :].to(input.dtype)
...
class Conv2d(CurveModule):
    ...
    def __init__(self, in_channels, out_channels, kernel_size, fix_points, stride=1,
        ...
        if out_channels % groups != 0:
            raise ValueError(f'out_channels must be divisible by groups, out_channels={out_channels}, groups={groups}')
...
class CurveNet(Module):
    ...
    def import_base_parameters(self, base_model, index):
        ...
        # l_p = list(base_parameters)
        # print(f"curve_len:{len(parameters)}, end_len:{len(list(base_parameters))}")
...
def base_l2_regularizer(weight_decay):
