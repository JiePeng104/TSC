...
from models.imagenet_model import ImageNetResNet_project, BasicBlock, Bottleneck
...
class ResNetCurve(nn.Module):
    def __init__(
        ...
        self.conv1 = curves.Conv2d(3, self.inplanes, kernel_size=7, fix_points=fix_points, stride=2, padding=3,
                                   bias=False)
...
class ImageNetResNetCurve_project(nn.Module):
    def __init__(self,
                 #  embed_dim: int,
                 #  # vision
        ...
        super().__init__()
        self.visual = ResNetCurve(BottleneckCurve, [3, 4, 6, 3], width_mult=1, fix_points=fix_points)

        self.projection_model = nn.Sequential(curves.Linear(2048, 512, bias=False, fix_points=fix_points),
    ...
    def forward(self, x, coeffs_t):
        ...
        feature = self.visual(x, coeffs_t)

        out = self.projection_model[0](feature, coeffs_t)
        out = self.projection_model[1](out, coeffs_t)
        out = self.projection_model[2](out)
        out = self.projection_model[3](out, coeffs_t)

        return F.normalize(feature, dim=-1), F.normalize(out, dim=-1)


class ResNet50Arc_imagenet:
    base = ImageNetResNet_project
    curve = ImageNetResNetCurve_project
    kwargs_base = {'feature_dim': 128}
    kwargs_curve = {'feature_dim': 128}
